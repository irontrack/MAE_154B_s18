...
for LC=1:12 
  for zi=1:length(z)
    qbA=0;
      for i=2:length(BInd)+1
          qbA=qbA+qb{LC,zi}(i)*BoomArea(i-1);
      end
    qB{LC,zi}(3)=M0(LC)+Load.VWy(LC,zi)*(Cx-SCx)-qbA;
  end   %Wingspan
end     %Load Cases
...
for i=1:length(BInd)
    if i==length(BInd)
     Sy=Sy+ShearFlow{LC,1}(i)*Spars.Length(2)*spart;
    else
     Sx=Sx+ShearFlow{LC,1}(i)*(B.posX(i+1)-B.posX(i))*skint;
     Sy=Sy+ShearFlow{LC,1}(i)*(B.posY(i+1)-B.posY(i))*skint;
    end
end
Sy=Sy+(ShearFlow12{LC,1}(1)+ShearFlow12{LC,1}(2))*Spars.Length(1)*spart;

disp('Shear Force in x-direction at the root in PLAA is (N):')
disp(Load.VWx(LC,1))
disp(Sx)
disp('Calculated Error with Shear Flow is (%);')
disp(abs(Load.VWx(LC,1)-Sx)/abs(Load.VWx(LC,1))*100)
disp('Shear Force in y-direction at the root in PLAA is (N):')
disp(Load.VWy(LC,1))
disp(Sy)
disp('Calculated Error with Shear Flow is (%);')
disp(abs(Load.VWy(LC,1)-Sy)/abs(Load.VWy(LC,1))*100)
%% Buckling Analysis
% Bending buckling 
rib_spacing_Rev6_9
% Shear buckling
